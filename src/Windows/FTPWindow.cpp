...
//Simple blob to trick SynWrite into thinking actual ftp data is attached. Not needed though, and the plugin doesnt use it ;)
char infoBlob[32];
const int infoSize = 32;

const TCHAR * FTPWindow::FTPWINDOWCLASS = TEXT("SYNFTPMAIN");

FTPWindow::FTPWindow() :
	Window(NULL, FTPWINDOWCLASS),
	DropTargetWindow(),
...
int FTPWindow::Create(HWND hParent, HWND hSyn, ACTIONPROC ActionProc) {
	m_hSyn = hSyn;
	...
	m_outputLog.RegisterActionProc(GetHWND(), ActionProc);
	...
	//Not sure if object pascal windows allow drag n drop
	m_dropHwnd = m_hwnd;
...
int FTPWindow::OnProfileChange() {
	...
	for(size_t i = 0; i < m_vProfiles->size(); i++) {
		...
		if (m_ftpSettings->GetMenuSplit() > 0 && i > 0 && (i % m_ftpSettings->GetMenuSplit()) == 0)
			flags |= MF_MENUBARBREAK;
		::AppendMenu(m_popupProfile, flags, IDM_POPUP_PROFILE_FIRST + i, m_vProfiles->at(i)->GetName());
	}
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case IDB_BUTTON_TOOLBAR_UPLOAD: {
					...
					int saveStatus = m_actionProc(GetHWND(), cActionSaveFile, (void*)cSynIdCurrentFile, (void*)true, NULL, NULL);
					if (saveStatus != cSynOK)
						return TRUE;
					...
					if ((state & 0x8000) && LOWORD(wParam) == IDB_BUTTON_TOOLBAR_UPLOAD) {
						...
						int res = m_actionProc(GetHWND(), cActionGetOpenedFileName, source, (void*)cSynIdCurrentFile, NULL, NULL);
						doUpload = (res == cSynOK && source[0] != 0);
					}
					if (doUpload == TRUE) {
				...
				default: {
					...
					if (!m_busy && value >= IDM_POPUP_PROFILE_FIRST && value <= IDM_POPUP_PROFILE_MAX) {
						...
						if (ret == -1) {
							OutErr("[SynFTP] Cannot start FTP session");
							result = TRUE;
							break;
						}
		...
		case QueueOperation::QueueTypeDownload: {
			...
			if (queueOp->GetType() == QueueOperation::QueueTypeDownload) {
				if (code == 0) {
					...
					m_actionProc(GetHWND(), cActionOpenFtpFile, (void*)opdld->GetLocalPath(), infoBlob, (void*)infoSize, NULL);
				} else {
					//Download to other location: Ask
					int ret = ::MessageBox(m_hSyn, TEXT("The download is complete. Do you wish to open the file?"), TEXT("Download complete"), MB_YESNO);
					if (ret == IDYES) {
						m_actionProc(GetHWND(), cActionOpenFtpFile, (void*)opdld->GetLocalPath(), infoBlob, (void*)infoSize, NULL);
					}
				}
...
int FTPWindow::OnConnect(int code) {
	...
	m_actionProc(GetHWND(), cActionSetState, static_cast<void*>(const_cast<TCHAR*>(m_ftpSession->GetCurrentProfile()->GetName())), NULL, NULL, NULL);
