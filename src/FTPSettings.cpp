...
int FTPSettings::GetMenuSplit() const {
	return m_menuSplit;
}
int FTPSettings::SetMenuSplit(int menuSplit) {
	m_menuSplit = menuSplit;
	return 0;
}
...
int FTPSettings::LoadSettings(const TiXmlElement * settingsElem) {
	...
	int menuSplit = 0;
	const char * splitstr = settingsElem->Attribute("menuSplit", &menuSplit);
	if (!splitstr) {
		menuSplit = 0;
	}
	m_menuSplit = menuSplit;
	...
	if (passstr) {
		...
		bool success = false;
		while(!success) {
			int res = inputPass.Create(_MainOutputWindow, TEXT("SynFTP: Master password required"), query, TEXT(""), true);
			if (res == 1) {
				char * localPass = SU::TCharToCP(inputPass.GetValue(), CP_ACP);
				char * challenge = Encryption::Decrypt(localPass, -1, passstr, true);

				if (strcmp(challenge, "SynFTP")) {
		...
		if (!success) {
			MessageBox(_MainOutputWindow,
							...
							TEXT("SynFTP: Password manager error"), MB_OK);
		}
...
int FTPSettings::SaveSettings(TiXmlElement * settingsElem) {
	...
	settingsElem->SetAttribute("menuSplit", m_menuSplit);

	if (!Encryption::IsDefaultKey()) {
		char * challenge = Encryption::Encrypt(NULL, -1, "SynFTP", -1);
